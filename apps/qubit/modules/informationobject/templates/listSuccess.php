  ...
  <thead>
    ...
    <?php foreach ($informationObjects as $item): ?>
      <tr class="<?php echo 0 == @++$row % 2 ? 'even' : 'odd' ?>">
        <td>
          <?php echo link_to(render_title($item), [$item, 'module' => 'informationobject']) ?><?php if (QubitTerm::PUBLICATION_STATUS_DRAFT_ID == $item->getPublicationStatus()->status->id): ?> <span class="publicationStatus"><?php echo render_title($item->getPublicationStatus()->status) ?></span><?php endif; ?>
        </td><td>
          <?php if (sfConfig::get('app_multi_repository')): ?>
